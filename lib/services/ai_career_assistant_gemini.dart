...
''';
  ...
  static CareerInsights _parseAIResponse(String aiResponse) {
    ...
    final recommendations = [
      ActionableRecommendation(
        ...
        description:
            'Leverage AI insights to optimize your professional profile and increase visibility to recruiters',
      ...
      ActionableRecommendation(
        ...
        description:
            'Focus on high-demand skills identified through AI market analysis',
  ...
  static List<ActionableRecommendation> _parseRecommendations(
      String aiResponse) {
  ...
  static Map<String, dynamic> _parseAnalysisResponse(
      String aiResponse, List<JobApplication> applications) {
    return _getFallbackAnalysis(applications);
  }
  ...
  static double _calculateAverageMood(List<mood_model.MoodEntry> moodEntries) {
    ...
    return moodEntries.map((e) => e.moodLevel).reduce((a, b) => a + b) /
        moodEntries.length;
  }
  ...
  static Map<String, dynamic> _getFallbackMoodAnalysis(
      List<mood_model.MoodEntry> moodEntries) {
    return {
      'optimalMoodRange': {'min': 3.5, 'max': 4.5},
  ...
  static List<String> _generateApplicationSuggestions(
      List<JobApplication> applications) {
    ...
    if (recentApps.length < 5) {
      suggestions.add(
          "🎯 AI recommends 3-5 applications per week for optimal results");
    }

    return suggestions;
  }

  static List<String> _getTopPerformingCompanies(
      List<JobApplication> applications) {
    if (applications.isEmpty)
      return ['Focus on tech startups', 'Consider remote-first companies'];

    final companySuccess = <String, double>{};
    for (final app in applications) {
      final success =
          app.status == 'Interview' || app.status == 'Offer' ? 1.0 : 0.0;
      companySuccess[app.company] =
          (companySuccess[app.company] ?? 0.0) + success;
    }
  ...
  static List<String> _generateMoodBasedTips(
      List<mood_model.MoodEntry> moodEntries) {
